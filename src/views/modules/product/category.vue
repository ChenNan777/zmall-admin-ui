<template>
  <div>
    ...
    <el-button v-show="isDrag" type="primary" plain size="small" @click="updateDrag">
      确认修改
    </el-button>
    <el-button type="danger" plain size="small" @click="batchRemove">批量删除</el-button>
    <el-tree
      :data="menus"
      v-loading="loading"
      :props="defaultProps"
...
export default {
  ...
  methods: {
    ...
    batchRemove() {
      ...
      this.$confirm(`此操作将删除 ${names}分类, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          console.log('删除节点:' + ids);
          this.$http({
            url: this.$http.adornUrl('/product/category/delete'),
            method: 'post',
            data: this.$http.adornData(ids, false)
    ...
    handleDrop(draggingNode, dropNode, dropType, ev) {
      ...
      //如果dropType为before或after，父节点就是dropNode的父节点 如果dropNode为根节点父节点id就为0
      //2、catLevel
      //如果dropType为inner，level就是dropNode的level+1
      //如果dropType为before或after，level就是dropNode的level
      //还需要修改子节点的level
      //3、sort
      //如果dropType为inner，需要排序的节点就是dropNode的所有子节点
      //如果dropType为before或after，需要排序的节点就是dropNode的父节点的所有子节点 如果dropNode为根节点就是所有根节点
      if (dropType == 'inner') {
      ...
      //将要更新的数据加入待提交区
      this.categoryDragChanges.forEach((item) => this.updateChanges.push(item));
      this.openNode.push(parentCid);
    },
    ...
    updateDrag() {
      console.log('拖拽结果' + this.updateChanges);
      this.$http({
        url: this.$http.adornUrl('/product/category/update/drag'),
        method: 'post',
        data: this.$http.adornData(this.updateChanges, false)
      }).then(({ data }) => {
        this.$message({
          type: 'success',
          message: '分类顺序修改成功'
        ...
        this.getMenus();
        //设置默认展开菜单
        this.openNode.forEach((item) => this.expanded.push(item));
        //关闭拖拽功能
        this.isDrag = false;
        console.log('拖拽结果提交成功');
      });
    }
  },
  created() {},
